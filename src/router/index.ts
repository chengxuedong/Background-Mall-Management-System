...
export function addRoutes(menus: Array<{ frontpath?: string; child?: any[] }>): boolean {
    let hasNewRoutes = false

    const findAndAddRoutesByMenus = (arr: Array<{ frontpath?: string; child?: any[] }>) => {
        arr.forEach((menu) => {
            const matched = asyncRoutes.find(route => route.path === menu.frontpath)
            if (matched && matched.name && !router.hasRoute(matched.name)) {
                router.addRoute("admin", matched)
                hasNewRoutes = true
            }
