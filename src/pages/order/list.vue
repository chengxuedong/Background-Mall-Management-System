...
<template>
    <div class="orderItem">
        <el-tabs v-model="searchForm.tab" @tab-change="getData">
            <el-tab-pane :lazy="true" :label="item.name" :name="item.key" v-for="(item, index) in tabbars" :key="index">
            </el-tab-pane>
        </el-tabs>
        <el-card shadow="never" class="border-0">
            ...
            <el-table :data="tableData" ref="multipleTableRef" @selection-change="handleSelectionChange" stripe
                ...
                <el-table-column label="商品" width="400" align="center">
                    <template #default="{ row }">
                        ...
                        <div class="flex py-2" v-for="(item, index) in row.order_items" :key="index">
                            <el-image style="width: 30px;height: 30px;" fit="fill" :lazy="true"
                                :src="item.goods_item ? item.goods_item.cover : ''"></el-image>
                ...
                <el-table-column label="实际付款" width="170" prop="total_price" align="center">
                </el-table-column>
                ...
                <el-table-column label="交易状态" width="200" align="center">
                    <template #default="{ row }">
                        ...
                        <div>
                            收货状态
                            <el-tag :type="row.ship_status ? 'success' : 'info'" size="small">
                ...
                <el-table-column label="操作" align="center">
                    <template #default="{ row }">
                        <el-button class="px-1" size="small" text @click="openInfoModal(row)">
                            订单详情
                        </el-button>
                        <el-button v-if="searchForm.tab == 'noship'" class="px-1" size="small" text @click="">
                            订单发货
                        </el-button>
                        <el-button v-if="searchForm.tab == 'refunding'" class="px-1" size="small" text
...
const handleRefund = (id, agree) => {
    (agree ? showModal("是否同意该订单退款？") : showPrompt("请输入拒绝的理由"))
        //showModal的话value拿不到值,showPrompt的话value可以拿到输入框的值
        .then(({ value }) => {
            let data = { agree };
            if (!agree) {
